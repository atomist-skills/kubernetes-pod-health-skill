...
describe("K8sPodState", () => {
    ...
    describe("handler", () => {
        ...
        it("detects init container failure", async () => {
            ...
            const c = generateContext(d, s, l);
            const r = await handler(c);
            const em = "Init container fail (docker.io/library/busybox:1.31.1-uclibc) of pod production/init-fail in Kubernetes cluster k8s-internal-demo has restarted too many times: `13 > 10`";
            const e = {
                code: 1,
                reason: em,
            };
